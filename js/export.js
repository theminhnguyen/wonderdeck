...
/* ===== Grußkarte (Game-Boy-Stil) — eigenständige Kopie von card.js.
   Bei Änderungen an card.js hier mitziehen (Doppelpflege wie WORLD_RUNTIME). ===== */
function CARD_RUNTIME(DECK) {
  const PAL = ["#e8f0f8", "#a8c0e0", "#5878a8", "#182840"], INK = "#182840", PAPER = "#f8f8f8";
  const SPR = {
  ...
  const npcVariant = (rows) => rows.map((r) => r.replace(/[12]/g, (c) => (c === "2" ? "1" : "2")));
  function renderSprite(rows, flip) {
    const c = document.createElement("canvas"); c.width = 16; c.height = 16; const x = c.getContext("2d");
    rows.forEach((row, ry) => { for (let rx = 0; rx < 16; rx++) { const ch = row[rx]; if (ch === "." || ch == null) continue; x.fillStyle = PAL[+ch]; x.fillRect(flip ? 15 - rx : rx, ry, 1, 1); } });
    return c;
  }
  function makeTile(draw) { const c = document.createElement("canvas"); c.width = 16; c.height = 16; draw(c.getContext("2d")); return c; }
  const T = {};
  T.floor = makeTile((x) => { x.fillStyle = PAL[0]; x.fillRect(0, 0, 16, 16); x.fillStyle = PAL[1]; x.fillRect(0, 15, 16, 1); x.fillRect(15, 0, 1, 16); x.fillRect(3, 3, 1, 1); x.fillRect(11, 9, 1, 1); });
  T.wall = makeTile((x) => { x.fillStyle = PAL[2]; x.fillRect(0, 0, 16, 16); x.fillStyle = PAL[3]; for (let y = 3; y < 16; y += 4) x.fillRect(0, y, 16, 1); x.fillRect(7, 0, 1, 4); x.fillRect(3, 4, 1, 4); x.fillRect(11, 4, 1, 4); x.fillRect(7, 8, 1, 4); x.fillRect(3, 12, 1, 4); x.fillRect(11, 12, 1, 4); x.fillRect(0, 0, 16, 1); });
  T.window = makeTile((x) => { x.drawImage(T.wall, 0, 0); x.fillStyle = PAL[3]; x.fillRect(2, 3, 12, 10); x.fillStyle = PAL[0]; x.fillRect(3, 4, 10, 8); x.fillStyle = PAL[1]; x.fillRect(3, 9, 10, 3); x.fillStyle = PAL[3]; x.fillRect(7, 4, 1, 8); x.fillRect(3, 7, 10, 1); });
  T.rug = makeTile((x) => { x.fillStyle = PAL[1]; x.fillRect(0, 0, 16, 16); x.fillStyle = PAL[2]; x.fillRect(0, 0, 16, 1); x.fillRect(0, 15, 16, 1); x.fillRect(0, 0, 1, 16); x.fillRect(15, 0, 1, 16); x.fillRect(4, 4, 2, 2); x.fillRect(10, 10, 2, 2); x.fillRect(10, 4, 2, 2); x.fillRect(4, 10, 2, 2); });
  T.plant = makeTile((x) => { x.drawImage(T.floor, 0, 0); x.fillStyle = PAL[3]; x.fillRect(5, 10, 6, 5); x.fillRect(4, 14, 8, 1); x.fillStyle = PAL[2]; x.fillRect(6, 11, 4, 3); x.fillRect(6, 2, 4, 8); x.fillRect(3, 4, 4, 5); x.fillRect(9, 4, 4, 5); x.fillStyle = PAL[3]; x.fillRect(7, 6, 2, 4); x.fillRect(5, 5, 1, 2); x.fillRect(10, 5, 1, 2); });
  T.table = makeTile((x) => { x.drawImage(T.floor, 0, 0); x.fillStyle = PAL[3]; x.fillRect(1, 4, 14, 9); x.fillStyle = PAL[1]; x.fillRect(2, 5, 12, 6); x.fillStyle = PAL[2]; x.fillRect(2, 9, 12, 2); x.fillStyle = PAL[0]; x.fillRect(4, 6, 3, 2); x.fillStyle = PAL[2]; x.fillRect(4, 6, 3, 1); });

  const mk = (cls, tag) => { const d = document.createElement(tag || "div"); d.className = cls; return d; };
  const overlay = mk("gbcard"); document.body.appendChild(overlay);
  const holder = mk("gbcard__holder"); overlay.appendChild(holder);
  const cv = document.createElement("canvas"); cv.width = 160; cv.height = 144; cv.className = "card__screen"; holder.appendChild(cv);
  const hint = mk("gbcard__hint"); hint.innerHTML = "Pfeile/WASD gehen · <b>E</b> sprechen"; overlay.appendChild(hint);
  const pad = mk("gbcard__pad"); overlay.appendChild(pad);
  const dpad = mk("gbcard__dpad"); pad.appendChild(dpad);
  const mkBtn = (id, txt, parent) => { const b = mk("", "button"); b.id = id; b.textContent = txt; (parent || dpad).appendChild(b); return b; };
  mkBtn("cardUp", "▲"); mkBtn("cardLeft", "◀"); mkBtn("cardRight", "▶"); mkBtn("cardDown", "▼");
  const aBtn = mk("gbcard__a", "button"); aBtn.textContent = "A"; pad.appendChild(aBtn);
  const ctx = cv.getContext("2d"); ctx.imageSmoothingEnabled = false;

  const sprites = { down: [renderSprite(SPR.down0), renderSprite(SPR.down1)], up: [renderSprite(SPR.up0), renderSprite(SPR.up1)], right: [renderSprite(SPR.side0), renderSprite(SPR.side1)], left: [renderSprite(SPR.side0, true), renderSprite(SPR.side1, true)] };
  const npcSprites = { down: renderSprite(npcVariant(SPR.down0)), up: renderSprite(npcVariant(SPR.up0)), right: renderSprite(npcVariant(SPR.side0)), left: renderSprite(npcVariant(SPR.side0), true) };

  const greet = (DECK.greetings || []).filter((g) => (g.text || "").trim() || (g.name || "").trim());
  const COLS = 10, ROWS = Math.max(9, 7 + Math.ceil(greet.length / 3) * 2);
  const solid = new Set(); const tiles = [];
  ...
  function wrap(text, width) { const words = String(text).split(/\s+/); const lines = []; let line = ""; for (const w of words) { const t = line ? line + " " + w : w; if (t.length > (width || 17) && line) { lines.push(line); line = w; } else line = t; } if (line) lines.push(line); return lines; }
  function openDialog(text) { const lines = wrap(text); D.pages = []; for (let i = 0; i < lines.length; i += 2) D.pages.push(lines.slice(i, i + 2)); D.page = 0; D.shown = 0; D.open = true; }
  function aPress() {
    if (!D.open) {
      const fx = P.x + (P.dir === "left" ? -1 : P.dir === "right" ? 1 : 0);
      const fy = P.y + (P.dir === "up" ? -1 : P.dir === "down" ? 1 : 0);
      const n = npcs.find((n) => n.x === fx && n.y === fy);
      if (n) { n.dir = P.dir === "up" ? "down" : P.dir === "down" ? "up" : P.dir === "left" ? "right" : "left"; const name = (n.g.name || "").trim(); openDialog((name ? name.toUpperCase() + ": " : "") + (n.g.text || "…")); }
      return;
    }
    const total = D.pages[D.page].join(" ").length + D.pages[D.page].length;
    if (D.shown < total) { D.shown = total; return; }
    if (D.page < D.pages.length - 1) { D.page++; D.shown = 0; } else D.open = false;
  }
  const keys = {};
  function dirFromKeys() { if (keys["arrowup"] || keys["w"]) return "up"; if (keys["arrowdown"] || keys["s"]) return "down"; if (keys["arrowleft"] || keys["a"]) return "left"; if (keys["arrowright"] || keys["d"]) return "right"; return null; }
  window.addEventListener("keydown", (e) => { const k = e.key.toLowerCase(); if (["arrowup", "arrowdown", "arrowleft", "arrowright", " "].includes(k)) e.preventDefault(); keys[k] = true; if (k === "e" || k === " " || k === "enter") aPress(); });
  window.addEventListener("keyup", (e) => { keys[e.key.toLowerCase()] = false; });
  const isTouch = window.matchMedia("(pointer: coarse)").matches;
  pad.hidden = !isTouch;
  const bindBtn = (b, key) => { const on = (e) => { e.preventDefault(); if (key === "e") aPress(); else keys[key] = true; }; const off = (e) => { e.preventDefault(); keys[key] = false; }; b.addEventListener("touchstart", on, { passive: false }); b.addEventListener("touchend", off); b.addEventListener("touchcancel", off); };
  bindBtn(document.getElementById("cardUp"), "arrowup"); bindBtn(document.getElementById("cardDown"), "arrowdown");
  bindBtn(document.getElementById("cardLeft"), "arrowleft"); bindBtn(document.getElementById("cardRight"), "arrowright"); bindBtn(aBtn, "e");
  function fit() { const s = Math.max(1, Math.floor(Math.min(window.innerWidth / 160, (window.innerHeight - (isTouch ? 150 : 0)) / 144))); cv.style.width = 160 * s + "px"; cv.style.height = 144 * s + "px"; }
  ...
  function loop() {
    frame++;
    if (!D.open) {
      if (!P.moving) { const d = dirFromKeys(); if (d) { P.dir = d; const nx = P.x + (d === "left" ? -1 : d === "right" ? 1 : 0); const ny = P.y + (d === "up" ? -1 : d === "down" ? 1 : 0); if (walkable(nx, ny)) { P.moving = true; P.mx = nx; P.my = ny; P.prog = 0; } } }
      if (P.moving) { P.prog += 1 / 10; if (P.prog >= 1) { P.x = P.mx; P.y = P.my; P.moving = false; P.prog = 0; } const fx = P.moving ? P.x + (P.mx - P.x) * P.prog : P.x, fy = P.moving ? P.y + (P.my - P.y) * P.prog : P.y; P.px = Math.round(fx * 16); P.py = Math.round(fy * 16); if (frame % 8 === 0) P.step = 1 - P.step; }
    ...
    for (const n of npcs) {
      ...
      if (Math.abs(n.x - P.x) + Math.abs(n.y - P.y) === 1 && !D.open) { ctx.fillStyle = INK; ctx.fillRect(n.x * 16 + 7, n.y * 16 - camY - 8, 2, 4); ctx.fillRect(n.x * 16 + 7, n.y * 16 - camY - 3, 2, 2); }
    }
    ctx.drawImage(sprites[P.dir][P.moving ? P.step : 0], P.px, P.py - camY - 2);
    if (D.open) {
      ctx.fillStyle = PAPER; ctx.fillRect(2, 102, 156, 40);
      ctx.fillStyle = INK; ctx.fillRect(2, 102, 156, 2); ctx.fillRect(2, 140, 156, 2); ctx.fillRect(2, 102, 2, 40); ctx.fillRect(156, 102, 2, 40);
      ...
      lines.forEach((ln, i) => { const part = ln.slice(0, Math.max(0, budget)); budget -= ln.length + 1; ctx.fillText(part, 10, 111 + i * 13); });
      const total = lines.join(" ").length + lines.length;
      if (D.shown >= total && frame % 30 < 18) { ctx.fillStyle = INK; ctx.fillRect(146, 132, 6, 2); ctx.fillRect(147, 134, 4, 1); ctx.fillRect(148, 135, 2, 1); }
    }
    requestAnimationFrame(loop);
  }
  const boot = () => { openDialog((DECK.title || "Grußkarte") + " — Sprich mit allen Leuten! (A = E-Taste)"); requestAnimationFrame(loop); };
  (document.fonts ? document.fonts.load("8px 'Press Start 2P'").catch(() => {}).then(boot) : Promise.resolve().then(boot));
}

const CARD_CSS = "*{margin:0;box-sizing:border-box}html,body{height:100%;overflow:hidden;background:#0c1422;font-family:Inter,sans-serif}"
  + ".gbcard{position:fixed;inset:0;display:grid;place-items:center}"
  + ".gbcard__holder{display:grid;place-items:center}"
  + ".card__screen{image-rendering:pixelated;image-rendering:crisp-edges;border-radius:6px;box-shadow:0 0 0 8px #182840,0 0 0 10px #0a0e14,0 30px 80px rgba(0,0,0,.6);background:#182840}"
  + ".gbcard__hint{position:fixed;bottom:16px;left:50%;transform:translateX(-50%);font-size:12px;color:rgba(255,255,255,.65);background:rgba(0,0,0,.4);padding:6px 14px;border-radius:999px}.gbcard__hint b{color:#fff}"
  + ".gbcard__pad{position:fixed;left:0;right:0;bottom:42px;display:flex;justify-content:space-between;align-items:center;padding:0 30px;pointer-events:none}.gbcard__pad[hidden]{display:none}"
  + ".gbcard__dpad{display:grid;grid-template-columns:48px 48px 48px;grid-template-rows:48px 48px;gap:2px;pointer-events:auto}"
  + ".gbcard__dpad button{font-size:17px;border-radius:10px;border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.1);color:#fff}"
  + ".gbcard__dpad #cardUp{grid-column:2;grid-row:1}.gbcard__dpad #cardLeft{grid-column:1;grid-row:2}.gbcard__dpad #cardRight{grid-column:3;grid-row:2}.gbcard__dpad #cardDown{grid-column:2;grid-row:2}"
  + ".gbcard__a{width:64px;height:64px;border-radius:50%;border:1px solid rgba(255,255,255,.3);background:#a83248;color:#fff;font-weight:700;font-size:19px;pointer-events:auto}";
...
function buildDoc(deck) {
  ...
  if (deck.mode === "card") {
    // Grußkarte: komplett eigenständig (nur Canvas + Pixel-Schrift), läuft offline.
    return docHead(deck, CARD_CSS)
      + "<body><script>(" + CARD_RUNTIME.toString() + ")(" + json + ");<\/script></body></html>";
  }
