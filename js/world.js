...
export async function openWorld(deck, resolveSrc, onClose = null) {
  ...
  for (let k = 0, z = -12; z > -hallLen + 10; z -= 20, k++) {
    ...
    const sh = k % 5; // abwechslungsreiche Skulpturen
  ...
  /* ----- Hängende Stoffbanner (von den Säulen in die offene Luft, gegen den Himmel) ----- */
  const bannerCols = [0xc0554b, 0x4d7a86, 0xcf9a4a, 0x6f7f9c]; // warme, gedämpfte Töne
  const bannerGeo = new THREE.PlaneGeometry(0.8, 1.6), bannerStripGeo = new THREE.PlaneGeometry(0.8, 0.16);
  const bannerStripMat = new THREE.MeshBasicMaterial({ color: acc.getHex() });
  disposables.push(bannerGeo, bannerStripGeo, bannerStripMat);
  for (let i = 0, z = -6; z > -hallLen + 8; z -= 14, i++) for (const sx of [-1, 1]) {
    const bx = sx * (halfW - 1.15), ry = -sx * Math.PI / 2; // vor den Säulen, zur Mitte gedreht
